...
- [x] 개발/테스트/운영 URL을 빌드 설정으로 분리한다. 운영 앱의 HTTPS 허용 목록을 유지하며 디버그에서 허용한 로컬 URL이 운영에 섞이지 않게 한다.
...
- [x] Vite가 만든 해시 기반 JS/CSS와 필수 화면 파일 목록을 빌드 시 생성해 사전 캐시한다. 응답 성공 여부를 확인하고 인증/개인 API 응답은 공용 캐시에 넣지 않는다.
- [x] HTML fallback은 화면 이동 요청에만 사용한다. JS/CSS/사진 요청에는 해당 형식의 캐시 또는 명확한 실패를 반환한다. 캐시 삭제도 이 앱이 소유한 접두사에만 한정한다.
- [x] 설정 로딩에는 제한 시간과 취소를 두고, 앱 모듈 import 실패도 잡는 초기 복구 화면을 React 바깥에 준비한다. 재시도·연결 확인은 가능하되 자동 데이터 삭제 버튼으로 처리하지 않는다.
- 네이티브 WebView의 메인 프레임 서버·파일 로드 실패는 네트워크 연결 여부와 무관하게 복구 화면으로 안내하며, 네트워크 단절과 서버 연결 실패를 구분한다. 실제 캐시 부팅 검증은 별도 기기 게이트로 남긴다.
