...
2026년 9월 7일 API 36 가상기기의 서명된 Release 앱에서 로그인하지 않은 상태로 `Persistence_0907` 테스트 일정을 생성했습니다. `am force-stop` 후 `LaunchState: COLD` 재실행과 APK의 `install -r` 업데이트 재설치를 각각 수행한 뒤에도 일정 카드가 복원되는 것을 화면에서 확인했습니다. 이 검증은 앱 데이터 삭제나 재설치가 아닌 실제 프로세스 종료 및 데이터 보존 업데이트 조건으로 수행했습니다.

같은 Release 앱에서 현재 위치 버튼을 눌렀을 때 Android 시스템 권한 창이 표시되고, 거절하면 앱의 위치 오류 안내가 나타나는 것을 확인했습니다. 이후 위치 권한을 다시 허용하고 가상 위치를 주입한 뒤 현재 위치 버튼을 다시 누르면 지도 중심에 파란 현재 위치 표시가 나타나 권한 복구 경로도 정상 동작했습니다.
