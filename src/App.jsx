...
const mapOptions = {
  ...
  mapId: 'DEMO_MAP_ID', // Required for advanced rendering and modern features
  styles: [
...
function App() {
  ...
  const [itinerary, setItinerary] = useState(() => {
    const saved = localStorage.getItem('world_travel_pro_v11');
  ...
  const saveItinerary = (newItinerary) => {
    ...
    localStorage.setItem('world_travel_pro_v11', JSON.stringify(newItinerary));
  };
  ...
  if (loadError) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-gray-50 text-gray-900 p-8 text-center">
        ...
        <p className="text-sm text-gray-500 mb-6 max-w-sm">Please check your API key and billing settings in Google Cloud Console.</p>
        <div className="p-4 bg-gray-100 rounded-2xl font-mono text-[10px] text-left">{loadError.message}</div>
      </div>
    );
  }
  ...
  if (!isLoaded) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-white">
        ...
        <p className="mt-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Initialising Map...</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-screen h-screen bg-gray-100 overflow-hidden font-sans">
      <AnimatePresence>
        {sidebarOpen && (
          <motion.aside 
            ...
            className="absolute top-6 left-6 bottom-6 w-[380px] bg-white/90 backdrop-blur-2xl rounded-[32px] shadow-2xl border border-white/50 flex flex-col overflow-hidden z-[1000]"
          >
            ...
            <div className="flex-1 overflow-y-auto px-8 custom-scroll pb-10">
               {viewMode === 'explore' ? (
                  filteredLocations.map(loc => (
                    ...
                    >
                      <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center text-2xl shadow-sm border border-gray-50">
                        {loc.emoji}
                      </div>
                      ...
                      <button 
                        ...
                        className="p-2 text-blue-600 hover:bg-blue-50 rounded-xl opacity-0 group-hover:opacity-100 transition-all"
                      >
                        <Plus size={18} />
                      </button>
                    </div>
                  ...
                  itinerary[0].items.map(item => (
                    <div key={item.id} className="flex items-center gap-4 p-4 bg-white border border-gray-100 rounded-2xl mb-2">
                       ...
                       <button onClick={() => removeFromItinerary(item.id)} className="p-2 text-gray-300 hover:text-red-500">
                          <Trash2 size={16} />
                       </button>
                    </div>
                  ))
      ...
      <main className="absolute inset-0 z-0">
        {!sidebarOpen && (
          <button onClick={() => setSidebarOpen(true)} className="absolute top-6 left-6 z-[2000] w-14 h-14 bg-white rounded-2xl shadow-2xl flex items-center justify-center text-blue-600 border border-white">
            <Menu size={24} />
          </button>
        )}
        ...
        <div className="absolute top-6 left-1/2 -translate-x-1/2 z-[2000] w-full max-w-sm px-4">
          <div className="bg-white/90 backdrop-blur-xl border border-white/50 rounded-3xl shadow-2xl p-2 flex items-center">
             ...
             <button className="w-11 h-11 bg-blue-600 text-white rounded-2xl flex items-center justify-center shadow-lg">
               <Search size={20} />
             </button>
          </div>
        </div>
                ...
                >
                  <div className="p-6 min-w-[260px]">
                    ...
                    <button 
                      ...
                      className="w-full py-4 bg-blue-600 text-white rounded-2xl text-xs font-black shadow-xl"
                    >
                      Add to Journey
                    </button>
                  </div>
